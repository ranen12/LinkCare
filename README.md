...
<!-- TODO: 아래는 예시 문구입니다. 실제 느낀 점으로 교체해주세요. -->

- **잘한 점**: 분기가 늘어나는 로직을 만났을 때 "분기를 하나씩 처리"하기보다 "분기가 생기는 구조 자체"를 없애는 방향으로 접근하는 습관을 들일 수 있었습니다.
- **아쉬운 점**: 혈당 조회 API의 그룹화 로직을 처음부터 서버 재조회 방식으로 설계했다면 프론트 머지 시도에 들인 시간을 줄일 수 있었을 것 같습니다.
- **다음에 시도해볼 것**: 상태 판정 로직(저혈압/정상/주의/경고/고혈압 등)에 대한 단위 테스트를 추가해 경계값 처리를 더 견고하게 만들고 싶습니다.
