...
달력 표현이 고민이었습니다. 혈압은 "더 위험한 쪽"으로 합쳐서 하나의 색으로 표시할 수 있었지만, 혈당은 식전 저혈당 + 식후 고혈당이 동시에 나올 수 있어 다른 방법이 필요했습니다.
그래서 달력 한 칸을 반원으로 나눠 왼쪽은 식전, 오른쪽은 식후 상태를 각각 다른 색으로 동시에 표현하는 방식으로 설계했습니다.

<img src="./assets/glucose-calendar.png" width="320" alt="혈당 반원 달력" />

반원은 식전, 식후가 서로 다른 이상 상태(저혈당·주의·고혈당·위험)로 겹칠 때만 표시됩니다. 정상+이상 상태, 이상 상태1+이상 상태2까지 전부 분할해서 보여주면 오히려 직관적으로 읽히지 않아서, 
"그날 문제가 있었는가"를 빠르게 확인할 수 있도록 했습니다.
...
결국 키는 프로필에서 한 번만 관리하도록 통일했고, 체중 등록 API의 DTO에서 `height` 필드 자체를 없앴습니다. 분기를 하나하나 처리하려니 끝이 안 보여서, 애초에 분기가 생기는 구조(키를 여러 곳에서 받는 것) 자체를 없애버린 셈입니다.
...
목표 설정 시 감량(`-`)/증량(`+`)/유지(`0`) 상태를 DB에 저장해두고, 새로운 목표체중을 입력할 때만 재계산하도록 바꿨습니다. 목표체중도 최초 1회만 체중 입력폼과 함께 받고, 이후엔 메인 화면에서 `PATCH /weight/profile`로 따로 수정할 수 있게 해서 매번 목표를 다시 입력하는 번거로움도 같이 없앴습니다.
...
오늘 기록을 추가로 입력하면 이미 조회해둔 7일 데이터를 다시 통째로 불러오는 게 아깝다고 생각해서, 처음엔 오늘 하루 데이터만 프론트에서 기존 목록에 머지하는 방식을 시도했습니다. 체중에서는 문제없이 동작했는데, 혈당에서 식전·식후 그룹화 로직까지 프론트 머지와 맞물리니까 복잡도가 확 올라가고 엣지 케이스가 계속 늘었습니다. 결국 체중·혈압·혈당 다 등록 후 서버에서 재조회하는 방식으로 통일했습니다. (참고로 날짜 기준 시각 계산도 처음엔 서버에서 처리했는데, 이후 팀 합의로 프론트에서 기준 시각을 넘겨주는 방식으로 바꿨습니다.)
